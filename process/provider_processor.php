...
if ($method == 'fetch_audited_list_fass_status') {
    ...
    WHERE  employee_num LIKE '$empid%' AND full_name LIKE '$fname%'  AND (date_audited >='$dateFrom' AND date_audited <= '$dateTo')  AND provider = '$esection' AND pd IS NOT NULL AND line_no LIKE '$lname%' AND date_sent IS NULL AND position LIKE '$position%' AND car_maker LIKE '$carmaker%' AND car_model LIKE '$carmodel%' AND audit_type LIKE '$audit_type%'
     GROUP BY id ORDER BY date_audited ASC";
...
if ($method == 'update_fass') {
    ...
    foreach($id as $x){
        ...
        $update = "UPDATE ialert_audit SET pd = '$status', date_sent = '' WHERE id = '$x'";
        $stmt = $conn->prepare($update);
        if ($stmt->execute()) {
            // echo 'approved';
            $count = $count - 1;
...
//               $count_na = "SELECT ialert_audit.date_audited, ialert_audit.provider,ialert_audit.position,ialert_audit.shift,ialert_audit.groups,
// ialert_audit.car_maker,ialert_audit.car_model,ialert_audit.line_no,ialert_audit.process,
// ialert_audit.audit_findings,ialert_audit.audited_by,ialert_audit.audited_categ,ialert_audit.audit_type,
// ialert_audit.audit_type,ialert_audit.remarks,ialert_audit.pd,ialert_audit.hr,ialert_audit.agency,
// ialert_audit.penalty,ialert_audit.date_created,ialert_audit.date_updated,
// ialert_section.line_no,ialert_section.car_maker,ialert_account.sections,count(ialert_audit.id) as total
// from ialert_audit
// left join ialert_section on ialert_audit.line_no = ialert_section.line_no
// left join ialert_account on ialert_account.sections = ialert_section.section
// where ialert_section.section = '$section' AND ialert_account.sections = '$section'
// AND ialert_audit.pd IS NULL AND ialert_audit.provider = 'FAS'";
...
if ($method == 'fetch_recieved_fas') {
    
    $c = 0;
    $select = "SELECT * FROM ialert_audit WHERE pd = 'IR' AND date_sent IS NOT NULL AND hr IS NOT NULL";
...
if ($method == 'fetch_recieve_fas') {
    ...
    $query = "SELECT * FROM ialert_audit
    WHERE  employee_num LIKE '$empid%' AND full_name LIKE '$fname%' AND car_maker LIKE '$carmaker%' AND car_model LIKE '$carmodel%'  AND line_no LIKE '$lname%' AND (date_audited >='$dateFrom' AND date_audited <= '$dateTo')  AND provider = '$esection' AND section = '$section' AND audit_type LIKE '$audit_type%' AND position LIKE '$position%' AND date_sent IS NOT NULL AND hr IS NOT NULL
